...
func TestGoCoverParser_Parse_InvalidInput(t *testing.T) {
	input := `invalid mode
file.go:5.10,7.2 2 1
`
...
func TestGoCoverParser_Parse_EmptyInput(t *testing.T) {
	input := ``
	...
	if report != nil {
		t.Error("Expected nil report for empty input")
	}
}

func TestGoCoverParser_Parse_InvalidNumbers(t *testing.T) {
	input := `mode: set
file.go:5.10,7.2 invalid 1
file.go:5.10,7.2 1 invalid
`
	...
	if report == nil {
		t.Error("Expected report even with invalid numbers")
	}

	warnings := parser.GetWarnings()
	if len(warnings) == 0 {
		t.Error("Expected warnings for invalid numbers")
	}
}

func TestGoCoverParser_Parse_MalformedLine(t *testing.T) {
	input := `mode: set
...
`
	...
	warnings := parser.GetWarnings()
	if len(warnings) == 0 {
		t.Error("Expected warnings for malformed line")
	}
}
